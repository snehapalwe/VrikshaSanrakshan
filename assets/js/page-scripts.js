...
$('#ctrl-application_type').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show();
	var ctrlVal = $(this).val();
	if(ctrlVal=="PARTIAL TREE CUT"){
	    $("#ctrl-cut_purpose").parents('.form-group').hide();
	    $("#ctrl-cut_purpose").removeAttr('required');
	}else{
	    $("#ctrl-cut_purpose").parents('.form-group').show();
	    $("#ctrl-cut_purpose").prop('required',true);
	}
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-application_id').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show();
	var ctrlVal = $(this).val();
        $.getJSON(siteAddr + 'api/get_app_form/' + ctrlVal.replace("SAT/TCP/2425/","") +  '/?csrf_token=' )
            .done(function (json) {
               $("#ctrl-no_of_tree_cut").val(json.no_of_trees);
               $("#ctrl-no_of_tree_planted").val(parseInt(json.no_of_trees)*2);
                if(json.amount>0 && json.application_type=="FULL TREE CUT"){
                }else{
                    alert("Not Eligible for a refund");
               $("#ctrl-no_of_tree_cut").val("");
               $("#ctrl-no_of_tree_planted").val("");
                }
            });
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-application_id').on('change', function(){ 
	...
	//$(#anotherfieldid).show();
	var ctrlVal = $(this).val();
        $.getJSON(siteAddr + 'api/get_app_form/' + ctrlVal +  '/?csrf_token=' )
            .done(function (json) {
               $("#ctrl-no_of_tree_cut").val(json.no_of_trees);
               $("#ctrl-no_of_tree_planted").val(parseInt(json.no_of_trees)*2);
                if(json.amount>0){
...
$('#ctrl-status').on('input', function () {
    var status = $(this).val();
    if (status === 'REJECT') {
        $('#ctrl-upload_rejection_note').show();   // unhide
    } 
    else if (status === 'ACCEPT') {
        $('#ctrl-upload_rejection_note').hide();   // hide
        $('#ctrl-upload_rejection_note').val('');  // clear value
    }
});
$('#ctrl-status').on('change', function () {
    if ($(this).val() == "REJECT") 
    {
        ...
        $("#ctrl-upload_rejection_note").val("0");
        $("#ctrl-upload_rejection_note").prop("required", false);
    }
});
$('#ctrl-cc_application_id').on('change', function() {
    // Get current year and compute the next year
    var today = new Date();
    var currentYear = today.getFullYear();
    var nextYear = (currentYear + 1).toString().slice(-2); // last 2 digits
    var financialYear = currentYear + '-' + nextYear;
    // Get the ID value
    var idVal = $('#ctrl-id').val();
    // Construct the required string
    var result = 'VVMC/CC/' + financialYear + '/' + idVal;
    // Set this value to your desired input field (for example, set back to cc_application_id)
    $('#ctrl-cc_application_id').val(result);
});
$('#ctrl-type_of_residence').on('change', function(){ 
    if ($(this).val() == "REJECT") {
        $("#ctrl-upload_rejection_note").closest(".form-group").show();
        $("#ctrl-upload_rejection_note").prop("required", true);
    } else {
...
$('#ctrl-demand').on('change', function(){ 
	//do something like 
	//$(this).hide(); 
	//$(#anotherfieldid).show();
	var ctrlVal = $(this).val();
	if(ctrlVal=="NO"){
	    $('#ctrl-amount').val(0);
	    $('#ctrl-amount').parents('.form-group').hide();
	}else{
	    $('#ctrl-amount').val("");
	    $('#ctrl-amount').parents('.form-group').show();
	}
	//$(#anotherfieldname).val(ctrlVal)
});
$('#ctrl-payment_mode').on('change', function(){ 
	var ctrlVal = $(this).val();
	if(ctrlVal=="CASH"){ 
	    $("#ctrl-chq_no").parents('.form-group').hide();
	    $("#ctrl-chq_date").parents('.form-group').hide();
	    $("#ctrl-bank").parents('.form-group').hide();
	    $("#ctrl-txnid").parents('.form-group').hide();
	    $("#ctrl-chq_date").removeAttr("required");
	    $("#ctrl-txnid").removeAttr("required");
	    $("#ctrl-bank").removeAttr("required"); 
	    $("#ctrl-txnid").removeAttr("required"); 
	}else if(ctrlVal=="ONLINE"){
		$("#ctrl-chq_no").parents('.form-group').hide();
	    $("#ctrl-chq_date").parents('.form-group').hide();
	    $("#ctrl-txnid").parents('.form-group').show();
	    $("#ctrl-txndate").parents('.form-group').show();
	    $("#ctrl-bank").parents('.form-group').hide(); 
	    $("#ctrl-chq_date").removeAttr("required"); 
	    $("#ctrl-bank").removeAttr("required");  
	    $("#ctrl-chq_no").removeAttr("required"); 
	    $("#ctrl-chq_date").removeAttr("required"); 
	}else{
	    $("#ctrl-bank").parents('.form-group').show();
	    $("#ctrl-chq_no").parents('.form-group').show();
	    $("#ctrl-chq_date").parents('.form-group').show();
	    $("#ctrl-chq_date").prop("required",true);
	    $("#ctrl-chq_no").prop("required",true); 
	    $("#ctrl-bank").prop("required",true); 
	}
});
});
/**
...
$(document).ready(function() {
    ...
    if(parseFloat(global_userrole)==2){ 
        $("#topbar ul:first").hide(); 
    }
});

function printSection(event) {
    var printButton = event.target;
    var container = printButton.closest('.col-sm-4'); // nearest .col-sm-4 parent
    if (!container) {
    ...
    // Print the window
    mywindow.print();
    // Close window after print dialog closes (if supported)
    mywindow.onafterprint = function() {
        mywindow.close();
    };
    return true;
}
function autoPrintImage(url) {
    const ext = url.split('.').pop().toLowerCase();
    const imageExts = ['jpg', 'jpeg', 'png', 'svg', 'webp', 'avif'];
    if (!imageExts.includes(ext)) return true; // Let PDFs or others open normally
