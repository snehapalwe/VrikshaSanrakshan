...
class Accept_rejectController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				...
				accept_reject.status LIKE ? OR 
				accept_reject.remark LIKE ? OR 
				accept_reject.timestamp LIKE ?
			)";
			$search_params = array(
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("db_name","rec_id","status","remark");
...
if($modeldata['status']=="ACCEPT"){
    if(USER_ROLE==7 && $rec['status']+0==1){
        $newstatus = "3";
    }
    if(USER_ROLE==7 && $rec['application_type']=="FULL TREE CUT"){ //full tree cut
        $newstatus = "3";
    }
    if(USER_ROLE==3 && $rec['status']+0==3){
        $newstatus = "4";
    }
    if(USER_ROLE==3  && $rec['application_type']=="FULL TREE CUT"){ //full tree cut
    $newstatus = "4.5";
    }
}else{
    $newstatus = "-2" ;
...
$db->update($db_name,["status"=>$newstatus]);
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","db_name","rec_id","status","remark");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'db_name' => 'required',
