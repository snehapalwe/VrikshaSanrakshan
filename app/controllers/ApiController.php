...
render_json($data);
	 ...
	 function offlineredirect(){
	     ...
	     if($allow){
	         ...
	         echo "<script>location.href='https://singlewindowsystemkdmc.in/offline_noc/integratelogin/?mob=tree&link=application_form';</script>";
	     }else{
	 ...
	 function cc_report(){
	    ...
	    $rec=$db->get("commencement_certificate",99999,"*,DATEDIFF(CURDATE(), current_timestamp) AS diff_days");
	    ...
	    foreach($rec as $r){
	        ...
	        }elseif(($r['status']+0==9 || $r['status']+0==11) && $r['permission_upload']==''){
	            $auth2++;
	            if($r['diff_days']+0>10){$p_wt_sla++;}else{$p_w_sla++;}
	        } elseif($r['status']+0==10){
	            $auth3++;
	            if($r['diff_days']+0>10){$p_wt_sla++;}else{$p_w_sla++;}
	        }elseif($r['permission_upload']!=''){
	            $complete++;
	 ...
	 function oc_report(){
	    ...
	    $rec=$db->get("occupancy_certificate",99999,"*,DATEDIFF(CURDATE(), current_timestamp) AS diff_days");
...
function viewreason($dbname,$id){
	    ...
	    $db->orderby('id','ASC');
	   // $reason =$db->getOne('accept_reject2',"*")['remark'];
	   $reason = $db->getOne('accept_reject2', "*");
	     ...
	     <body>
	         <table class="table">
	             ...
	             <tr>
	                 ...
	                 <td>
                        <?php 
                        if (is_array($reason['remark'])) {
                            echo implode(', ', $reason['remark']); // join array values
                        } else {
                            echo $reason['remark'];
                        }
                        ?>
	             ...
	             <?php if( ($reason['upload_rejection_note'] != "") && ($reason['status'] == "REJECT") ) { ?>
	             <tr>
	                 <th>Rejection Note</th>
	                 <td><a target="_blank" href="<?php echo $reason['upload_rejection_note']; ?>">VIEW REJECTION NOTE</a></td>
	             </tr>
	             <?php } ?>
	         </table>
	     </body>
...
function old_viewreason($dbname,$id){
	    $db=$this->GetModel();
	    $db->where("rec_id",$id);
	    $db->where("db_name",$dbname);
	    $db->orderby('id','ASC');
	 ...
	 <html>
	     <body>
	         <table class="table">
	             ...
	             <?php if( $reason['upload_rejection_note']!= ""){ ?>
	             <tr>
	                 <th>Upload</th>
	                 <td>  <a target="_blank" href="<?php echo $reason['upload_rejection_note']; ?>">VIEW REJECTION NOTE</a></td>
	             </tr>
...
$rec = $db->rawQuery("
    SELECT
        ...
        SUM(CASE WHEN status = 2 OR status = 7 THEN 1 ELSE 0 END) AS ci_count,
        SUM(CASE WHEN (status = 9 OR (status = 11 AND permission_upload='')) THEN 1 ELSE 0 END) AS gs_count,
        SUM(CASE WHEN status = 13 THEN 1 ELSE 0 END) AS gs_tippni_upload,
        SUM(CASE WHEN status = 10 THEN 1 ELSE 0 END) AS dmc_count,
        SUM(CASE WHEN status = 11 AND permission_upload!='' THEN 1 ELSE 0 END) AS completed,
        SUM(CASE WHEN status = -100 THEN 1 ELSE 0 END) AS reverted,
...
<body class="p-4 bg-light">
  ...
  <div class="container-fluid">
    <a class="navbar-brand fw-bold" href="#">KDMC Tree NOC Reports</a>
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav ms-auto">
        <li class="nav-item">
                    <a class="nav-link active" href="get_report">TREE CC</a>
                  </li><li class="nav-item">
                    <a class="nav-link " href="get_report2">TREE OC</a>
                  </li> 
                  </ul>
    </div>
  </div>
                ...
                <table class="status-table text-center mb-0">
                    <thead>
                        <tr>
                            ...
                            <th style="width:30%;background: #005bb5;">Count</th>
                        </tr>
                    </thead>
                    <tbody>
                        <!--<tr>-->
                        ...
                        <tr>
                            ...
                            <td><span class="badge bg-purple"><?= $rec['ci_count'] ?></span></td>
                        </tr>
                        <tr>
                            <td>Garden Superintendent</td>
                            <td><span class="badge bg-warning"><?= $rec['gs_count'] ?></span></td>
                        </tr>
                       
                        <tr>
                            <td>Chief Garden Superintendent</td>
    ...
    <script>
    const baseColors = [
      "#6f42c1", "#ffc107", "#0dcaf0", "#198754",
      "#dc3545", "#edc948", "#b07aa1", "#ff9da7"
    ];
...
<body>
    ...
    <div class="container">
        ...
        <div class="row fade-up text-center">
            <div class="col-2">
            </div>
            <div class="col-8">
                <div class="chart-container">
    ...
    <!-- Chart.js Script -->
    <script>
        const labels = ['Citizen', 'GI', 'GS', 'GS Tippni Upload', 'CGS', 'Completed', 'Rejected', 'Cancelled'];
        const values = [
            <?= $rec['citizen_count'] ?>,
            <?= $rec['cg_count'] ?>,
            <?= $rec['gs_count'] ?>,
