...
class OrdersController extends SecureController{
	...
	function index($fieldname = null , $fieldvalue = null){
		...
		if(!empty($request->search)){
			...
			$search_condition = "(
				orders.id LIKE ? OR 
				orders.rec_id LIKE ? OR 
				orders.order_date LIKE ? OR 
				orders.upload_order LIKE ? OR 
				orders.timestamp LIKE ?
			)";
			$search_params = array(
	...
	function add($formdata = null){
		if($formdata){
			...
			$request = $this->request;
			//fillable fields
			$fields = $this->fields = array("rec_id","order_date","upload_order");
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'rec_id' => 'required',
				'order_date' => 'required',
				'upload_order' => 'required',
			);
			$this->sanitize_array = array(
				'rec_id' => 'sanitize_string',
				'order_date' => 'sanitize_string',
				'upload_order' => 'sanitize_string',
			);
...
$db->update("application_form",["flag_order"=>$rec_id,"status"=>3]);
	...
	function edit($rec_id = null, $formdata = null){
		...
		$tablename = $this->tablename;
		 //editable fields
		$fields = $this->fields = array("id","rec_id","order_date","upload_order");
		if($formdata){
			$postdata = $this->format_request_data($formdata);
			$this->rules_array = array(
				'rec_id' => 'required',
