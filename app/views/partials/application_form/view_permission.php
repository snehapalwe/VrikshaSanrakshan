            ...
            <div class="row ">
                <div class="col-md-12 comp-grid">
                    ...
                    <div  class="card animated fadeIn page-content">
                        ...
                        $name="PARTIAL_PERMISSION";
                        }else{
                        if($data['cut_purpose']=="CONSTRUCTION PURPOSE"){
                        $name="FULL_CONSTRUCT_PERMISSION";
                        }else{
                        $name="FULL_DANGEROUS_PERMISSION";
                        }
                        }
                        $db->where("name",$name);
                        ...
                        $db->where("id",$data['demand_id']);
                        $payment=$db->getOne("payments","*");
                        foreach($payment as $k=>$d){
                        if(date("Y",strtotime($d))>2010 && str_contains('-',$d)){
                        if(date("H",strtotime($d))>0){
                        $string=str_replace("{{getval.demand.$k}}",date("d-m-Y h:i:s A",strtotime($d)),$string); 
                        $string=str_replace("{{getval.demand.dateonly_$k}}",date("d-m-Y",strtotime($d)),$string); 
                        }else{ 
                        $string=str_replace("{{getval.demand.$k}}",date("d-m-Y",strtotime($d)),$string);
                        }
                        }else{
                        $string=str_replace("{{getval.demand.$k}}",$d,$string);
                        }
                        }
